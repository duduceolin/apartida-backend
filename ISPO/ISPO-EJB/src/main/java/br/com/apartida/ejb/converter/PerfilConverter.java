...
import br.com.apartida.bean.perfilservice.FuncionalidadePerfilDTO;
...
public abstract class PerfilConverter extends AbstractConverter {
    ...
    public static Perfil cadastrarAlterarToEntity(InCadastrarAlterarPerfil in) throws NullPointerException {
        Perfil perfil = new Perfil();

        if (null != in.getId()) {
            perfil.setId(in.getId());
        }
        perfil.setIsAdmin(in.getAdmin());
        perfil.setNome(in.getNome());
        perfil.setEmpresa(new Empresa());
        perfil.getEmpresa().setId(in.getIdEmpresa());
